...
export default function HarvardCV({ data, isEditMode = false, onFieldChange, accentColor }: HarvardCVProps) {
  const em = isEditMode
  const ofc = onFieldChange
  const accent = accentColor || '#1F3A5F'

  const ruleLine = (
    <div style={{ borderTop: `1.5px solid ${accent}`, marginBottom: '6px', marginTop: '2px' }} />
  )
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', marginBottom: '24px', borderBottom: `1.5px solid ${accent}`, paddingBottom: '16px' }}>
        ...
        {data.titulo && (
          <p style={{ fontSize: '13pt', color: accent, fontWeight: 600, marginTop: '4px' }}>{data.titulo}</p>
        )}
        {data.contacto && (
            ...
            {data.experiencias.map((exp, idx) => (
              <div key={idx} style={{ breakInside: 'avoid' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  ...
                  <span style={{ fontSize: '10pt', color: '#595959' }}>
                ...
                {exp.bullets && exp.bullets.length > 0 && (
                  <ul style={{ margin: '4px 0 0 16px', padding: 0, listStyle: 'disc' }}>
                    {exp.bullets.map((b, bi) => (
                      <li key={bi} style={{ fontSize: '10.5pt', color: '#222', marginBottom: '2px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {data.educacion.map((edu, idx) => (
              <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', breakInside: 'avoid' }}>
                <div>
                  <EditableField tag="span" value={edu.titulo} path={`educacion.${idx}.titulo`}
                    isEditMode={em} onFieldChange={ofc}
                    style={{ fontWeight: 700, fontSize: '11pt', color: '#000' }} />
                  <span style={{ fontSize: '11pt', color: '#000' }}>, {edu.institucion}</span>
                  {shouldShowArea(edu.titulo, edu.area) && (
                    <p style={{ fontSize: '10pt', color: '#595959', marginTop: '2px' }}>{edu.area}</p>
                  )}
                </div>
        ...
        <div>
          ...
          <ul style={{ margin: '4px 0 0 16px', padding: 0, listStyle: 'disc' }}>
            {data.logros.map((logro, idx) => (
              <li key={idx} style={{ fontSize: '10.5pt', color: '#222', marginBottom: '2px', breakInside: 'avoid' }}>
