...
2. ⬜ **Verificación de Google OAuth** — hoy solo funciona para usuarios de prueba agregados a mano en Google Cloud; falta enviar a verificación para que cualquier usuario real pueda usarlo. Estaba bloqueado por contenido legal placeholder (ítem 4) — ya desbloqueado, falta hacer el envío real a revisión de Google con la URL de `/privacidad`.
...
4. ✅ **Contenido legal y acuerdo de embajadores** — **RESUELTO (2026-09-22).** Se redactó contenido real y sustantivo (no placeholder estructural) para `/privacidad` (12 secciones: datos recopilados, uso, proveedores reales — Anthropic/Supabase/NOWPayments/PayPal/Apify/Cloudflare/Upstash/Vercel/Google/LinkedIn —, seguridad, retención, derechos, cookies, prevención de abuso, menores, cambios), `/terminos` (11 secciones: servicio, cuenta, principio de no invención, planes y pagos —incluye cancelación inmediata sin prorrateo—, embajadores, propiedad intelectual, responsabilidad, terminación, cambios), `/cookies` (6 secciones, cookies reales usadas: sesión Supabase + `momentum_ref`) y `/uso-de-ia` (7 secciones). También se reescribió `TEXTO_ACUERDO_EMBAJADORES` en `app/(app)/ambassador/page.tsx` con las reglas reales del programa (25% comisión por defecto, holdback de 24h, máx. 6 meses de recurrencia, retiro en cripto sujeto a revisión manual, causales de suspensión) leídas directamente de `lib/embajadores.ts` y `scripts/migration-embajadores.sql`, en vez de texto genérico inventado. `LegalLayout` ganó un componente `Section` para prosa real. Verificado: `tsc --noEmit` limpio, las 4 páginas devuelven 200 y renderizan el contenido real, suite de tests sigue en 14/14. Desplegado a producción y confirmado en vivo en `www.momentumcv.com`.
   - **Actualización (2026-09-23):** el CEO dio el correo de contacto (`contact@momentumcv.com`) y la edad mínima (18 años, decisión tomada porque PayPal ya exige 18+ para tener cuenta y una suscripción es un contrato — no tenía sentido poner 16 sin poder hacerlo cumplir). Se reemplazaron los placeholders de correo/edad en `/privacidad`, `/cookies` y `/uso-de-ia`. También se detectó y arregló que **la homepage (`app/page.tsx`) no tenía ningún link a `/privacidad` ni `/terminos`** en su footer — Google exige que la política de privacidad sea encontrable desde la home real de la app, así que se agregó una columna "Legal" al footer con los 4 links. Verificado con Playwright real (no solo `curl`, porque la home es `'use client'` con gate de sesión y `curl` solo ve el spinner de "Cargando..."): los 4 links renderizan con los `href` correctos y 0 errores de consola. Todo verificado también en producción tras el deploy. Solo queda pendiente un placeholder puro de datos legales de la empresa (razón social/dirección/jurisdicción) en `/privacidad` y `/terminos`, que no bloquea la revisión de Google.
   - Nota para el ítem 5 de esta misma lista: se confirmó en este trabajo, verificando directamente el bundle JS servido por producción, que **el proyecto Supabase realmente en uso hoy es `otaehqzjwrhdkzdfchhr`** (coincide con `.env.local` y con lo guardado en "Dominios autorizados" de Google Cloud). Esto es evidencia fuerte de que `tqgupcgocnjtbrvyxkjd` (mencionado como "proyecto viejo" en el ítem 5) ya no es el que sirve producción y es un buen candidato a auditar/eliminar — pero no se ha confirmado su contenido, no eliminar sin auditar primero.
5. ⬜ **Proyecto viejo de Supabase (`tqgupcgocnjtbrvyxkjd`)** — auditar contenido y decidir si se elimina.
6. ✅ **Bug real: import de LinkedIn crasheaba para todos los usuarios** — **RESUELTO (2026-09-23).** Reportado por el CEO al intentar importar su propio perfil (`linkedin.com/in/abel-colmenares-a10017230`), mostraba el error genérico "No pudimos importar tu perfil de LinkedIn". Causa real: el actor de Apify `harvestapi/linkedin-profile-scraper` cambió `topSkills` de string (formato viejo, separado por "•") a array — y como un array vacío `[]` también es "truthy" en JS, `consolidateSkills()` en `app/api/profile/hydrate/route.ts` llamaba `.split('•')` sobre un array y crasheaba con `TypeError`, **para cualquier perfil**, no solo el del CEO. El error no estaba atrapado en ningún try/catch, así que tiraba un 500 crudo. Fix: `consolidateSkills()` ahora soporta ambos formatos (string viejo y array nuevo de `{name}`), y además empezó a leer el campo `skills` (con `{name, positions}`) que el actor ya devuelve y que antes se ignoraba por completo — antes de este fix, ningún import de LinkedIn traía skills reales, solo crasheaba. Se envolvió también la llamada síncrona a `processProfile()` en el handler POST en try/catch, para que un futuro cambio de esquema de este proveedor externo degrade al mensaje de error existente en vez de un 500 sin manejar. Verificado reproduciendo el bug real contra el perfil exacto del CEO (confirmado el crash con el código viejo, confirmado que ya no crashea y extrae 20 skills reales con el código nuevo, y que el pipeline completo hasta Anthropic funciona). Desplegado y confirmado por el CEO probándolo en producción con su cuenta real.
   - **Actualización (2026-09-23) — el "pendiente menor" resultó ser un bug real de invención de datos, no solo redacción rara.** El CEO compartió el PDF generado. Comparando línea por línea el texto real de su LinkedIn (Digital Finance International LLC) contra los bullets generados, se encontraron 2 resultados inventados que NO están en ninguna parte de la fuente: "...consolidando el ciclo de renovaciones contractuales **sin interrupción operativa**" (fuente real solo dice "control de renovaciones y recordatorios de pago", sin ninguna afirmación sobre continuidad operativa) y "...**reduciendo el tiempo de análisis** previo a reuniones ejecutivas" (fuente real solo dice "preparación de investigaciones/briefs", sin ninguna cifra ni afirmación de reducción de tiempo). Un tercer caso similar apareció en "Trabajo Especial de Grado": "...hasta su **aprobación y cierre formal**" (no está en la fuente). Causa raíz: la regla `QUANTIFICATION` en `lib/cv/prompts/global-rules.ts` (compartida por los pipelines general y de vacante, vía `buildGenerateCVPrompt`) le pide al modelo "cerrar cada bullet con un resultado cualitativo fuerte" sin prohibir explícitamente que ese resultado sea inventado cuando la fuente es puramente descriptiva de tareas (sin resultado explícito) — entra en tensión directa con la regla `DATA INTEGRITY` de la misma prompt, y el modelo resolvía la tensión inventando un resultado plausible. El validador `lib/cv/validation/anti-hallucination.ts` **no puede** atrapar esto por diseño (su propio docstring lo dice: solo detecta empresas/cargos/títulos inventados vía fuzzy matching, no afirmaciones fabricadas dentro de un bullet de texto libre). Fix: se agregó una sección nueva "THE QUALITATIVE OUTCOME ITSELF MUST NEVER BE INVENTED" a `global-rules.ts`, con los 2 ejemplos reales de este caso como contra-ejemplos explícitos (el mismo patrón que ya usa el archivo para la regla de persona gramatical) y un test explícito: "¿podría este resultado ser falso sin contradecir la fuente? Si sí, es inventado." Verificado de punta a punta: se reconstruyó el prompt real (`buildGenerateCVPrompt` + `buildGlobalRules`) con los datos reales de Digital Finance del CEO y se llamó al mismo modelo (Sonnet) dos veces — con el prompt viejo reproduce los mismos 2 inventos exactos; con el prompt nuevo, los mismos dos bullets ahora terminan en descripciones fieles a la fuente ("gestionando el ciclo completo de alta de servicios, renovaciones y recordatorios de pago" / "cubriendo el ciclo de análisis previo a reuniones de dirección"), sin inventar resultados. Este bug afecta a **todos** los usuarios cuya experiencia en LinkedIn está descrita en términos de tareas sin resultados explícitos (el caso más común), no solo al CEO — es un fix de alto impacto, no cosmético.
7. ✅ **Reescribir los textos legales — son demasiado explícitos sobre el stack interno** — **RESUELTO (2026-09-24).** El CEO revisó `/privacidad`, `/terminos`, `/cookies` y `/uso-de-ia` y pidió reglas explícitas: Apify es tema tabú (nunca nombrarlo ni describir qué hace), nunca afirmar que se usa un scraper para LinkedIn, en ningún lugar decir que se importa información de LinkedIn (ni el mecanismo ni que la función existe), y en general no nombrar qué proveedor/herramienta se usa para cada función interna. Se reescribieron las 4 páginas: se eliminaron todas las menciones a Anthropic, Apify, NOWPayments, Cloudflare, Supabase y Upstash, reemplazándolas por lenguaje genérico ("proveedores externos de infraestructura y tecnología", "tecnología de inteligencia artificial", "pasarela de pago externa", "verificación automatizada"). Google y LinkedIn se mantienen nombrados **únicamente** como opciones de inicio de sesión (nunca en relación a importar/scrapear datos) — es una mención estándar de "inicia sesión con X" que no revela nada de la arquitectura interna. La función de importar perfil de LinkedIn por URL ya no se menciona en ningún texto público; quedó implícitamente cubierta por el lenguaje genérico de "documentos que subes con tu información profesional". `TEXTO_ACUERDO_EMBAJADORES` (`app/(app)/ambassador/page.tsx`) se revisó y no necesitó cambios — nunca nombró proveedores. Verificado: `tsc --noEmit` limpio, grep case-insensitive contra las 4 páginas confirma cero ocurrencias de los nombres de proveedor, suite de tests 14/14. **Nota sin resolver, señalada pero sin decisión explícita del CEO:** ocultar por completo la fuente de datos de LinkedIn puede generar tensión con requisitos de transparencia de GDPR/CCPA sobre divulgar de dónde viene la información recopilada — se le señaló esto explícitamente antes de escribir el texto, y dio la instrucción de todas formas sin comentar sobre el riesgo puntualmente. Queda como riesgo residual conocido, no como pendiente técnico — vale la pena confirmarlo con él directamente en algún momento, no asumir que fue una decisión consciente solo porque no objetó.
8. ✅ **Bug real de negocio: generar CV gratis indefinidamente vía borrar-y-regenerar** — **RESUELTO (2026-09-24).** El CEO, pensando como ingeniero de producto, preguntó si un usuario del Plan Inicio podía crear un CV, no pagarlo, borrarlo, y crear otro indefinidamente sin pagar nunca. Confirmado: sí — `user_has_unpaid_cv` (el bloqueo existente) se deriva de las filas que existen HOY en `cvs`/`cvs_inspiracion`; al borrar la fila, el chequeo da "no tiene nada sin pagar" y deja generar otro gratis. Peor aún: esto era el comportamiento **documentado** en Términos ("...debes descargar (pagar) el anterior o eliminarlo primero").
   - **Opciones descartadas explícitamente con el CEO:** un límite que se reinicia por día/semana (sigue siendo gratis infinito a largo plazo, solo más lento) y el modelo de "trial cobra $X y a los 14 días cobra el resto hasta el precio completo" estilo CVwizard (riesgo legal real — coincide con el patrón de "dark pattern" que la FTC persigue activamente; sin entidad legal registrada, ese riesgo cae directo sobre el CEO como persona; y de todas formas no resuelve el problema de Plan Inicio, es una táctica de conversión a Pro).
   - **Diseño implementado:** límite de generaciones **de por vida** (no por día/mes) para cualquier usuario que **nunca** ha hecho un pago confirmado. El contador vive en `cv_generaciones_log`, tabla de solo-inserción escrita por un **trigger de base de datos** (`AFTER INSERT` en `cvs` y `cvs_inspiracion`) — no por código de la app — así que ni borrar el CV ni saltarse una API reduce el contador. Límite configurable en `configuracion.limite_generaciones_gratis` (hoy: 2). Función `user_free_generations_exhausted(uuid)`, `SECURITY DEFINER`. En el momento que el usuario tiene **cualquier** pago confirmado (uno solo, para siempre), sale de esta regla y vuelve a la regla normal de `user_has_unpaid_cv`.
   - **Cierra el callejón sin salida:** sin más cambios, alguien que agota su límite y borra su último CV se queda sin nada que pagar. Por eso también se bloquea el **DELETE** (no solo la generación) una vez agotado el límite mientras el CV siga sin pagar — queda "atrapado", garantizando que siempre haya algo que pagar para desbloquearse. Aplica en `cvs_inspiracion` (política RLS) y en `cvs` (política RLS).
   - **Hallazgo durante la verificación — la migración inicial NO funcionó en `cvs` la primera vez.** El script buscaba y reemplazaba políticas de DELETE por nombre/comando en `cvs`, pero la política real ahí era `own_cvs`, `FOR ALL` (cubre SELECT/INSERT/UPDATE/DELETE a la vez) — el script filtraba por `cmd = 'DELETE'` y nunca la encontró. Las políticas permisivas de Postgres se combinan con OR: `own_cvs` seguía permitiendo el borrado sin restricción en paralelo con la política nueva, anulándola. Se detectó con una prueba real end-to-end (no se asumió que funcionaba), y se corrigió con `scripts/migration-limite-generaciones-gratis-fix.sql`: reemplaza `own_cvs` por políticas explícitas de SELECT/UPDATE/DELETE. Efecto colateral positivo: `own_cvs` también permitía INSERT sin restricción — nadie insertaba nunca así (grep confirmó cero inserciones a `cvs` desde el cliente en toda la app), así que la política nueva **no** incluye INSERT, cerrando una vía que permitía a cualquier usuario autenticado insertar una fila de `cvs` falsa directo desde el navegador, saltándose la generación real por IA.
   - **Verificado con pruebas reales contra la base de datos de producción** (usuarios desechables creados y eliminados en la misma prueba, nunca se tocó una cuenta real): ciclo completo crear→borrar→crear→(bloqueo al agotar límite)→pagar→(desbloqueo total) en `cvs_inspiracion` y en `cvs`; aislamiento entre usuarios intacto (uno no puede leer/borrar CVs de otro); SELECT/UPDATE propios sin regresión; inserción falsa desde el cliente ahora bloqueada. `tsc --noEmit` limpio, suite de tests 14/14.
   - **Código de la app también corregido:** `app/api/generate-cv/route.ts` bloquea antes de gastar tokens de IA con el mensaje "Ya alcanzaste el límite de CVs gratis. Hazte Pro para continuar creando CVs y encuentra ese trabajo deseado." (texto exacto pedido por el CEO). `app/(app)/cvs/page.tsx`: los botones de borrar CV/CV Studio tenían un bug independiente — no comprobaban si el `DELETE` realmente afectó una fila; si RLS lo bloqueaba, Postgrest no da error (0 filas, sin excepción), y la UI igual lo quitaba de la lista como si se hubiera borrado, mintiéndole al usuario. Ahora se usa `.select('id')` tras el delete para confirmar antes de actualizar el estado local. `app/(app)/create-cv/studio/page.tsx`: chequeo previo vía RPC antes de intentar crear, para mostrar el mensaje correcto en vez de dejar pasar el texto crudo de Postgres ("new row violates row-level security policy...").
   - **Texto legal corregido a pedido del CEO:** Términos ya no dice "...o eliminarlo primero" — ese texto documentaba exactamente el mecanismo que un usuario podría explotar. Ahora dice que el Plan Inicio "tiene un número limitado de CVs que puedes generar sin pagar", sin mencionar el mecanismo de borrado en absoluto. Si un usuario lo descubre por su cuenta, no es porque lo haya leído en los términos.
   - **Limitación conocida y aceptada:** el historial de CVs ya borrados *antes* de este fix no se puede recuperar (se borraron de verdad, sin rastro) — el backfill solo contó lo que existía en el momento de correr la migración (67 filas). No afecta el cierre hacia adelante.
