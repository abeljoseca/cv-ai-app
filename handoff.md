...
7. ⬜ **Reescribir los textos legales — son demasiado explícitos sobre el stack interno (2026-09-23).** El CEO revisó `/privacidad`, `/terminos`, `/cookies` y `/uso-de-ia` y pidió cambiarlos a fondo: son extremadamente explícitos sobre información que debe ser confidencial. Reglas explícitas que dio:
   - **Apify es tema tabú — nunca mencionarlo en ningún texto público**, ni por nombre ni describiendo qué hace.
   - **Nunca afirmar que Momentum usa un scraper para obtener datos de LinkedIn.**
   - **En ningún lugar debe decir que se importa información de LinkedIn** — ni el mecanismo, ni que la función existe.
   - Más en general: no listar ni describir qué herramienta/proveedor se usa para cada función interna, ni cómo se usa — el nivel de detalle actual (que nombra Anthropic, Supabase, NOWPayments, PayPal, Apify, Cloudflare, Upstash, Vercel por nombre y explica su rol) es "extremadamente delicado" y hay que bajarlo mucho.
   - Pendiente para mañana: reescribir las 4 páginas (y probablemente `TEXTO_ACUERDO_EMBAJADORES`) con un nivel de divulgación mucho más genérico — probablemente hablando de categorías de tratamiento de datos sin nombrar proveedores específicos, y sin mencionar la función de importar LinkedIn en absoluto. **Ojo:** esto puede generar tensión real con requisitos de transparencia de GDPR/CCPA sobre divulgar fuentes de datos de terceros — vale la pena señalarlo en la conversación de mañana, pero la decisión final es del CEO.
