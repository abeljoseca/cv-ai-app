...
/* ══════════════════════════════════════════════════════════════════════════
   PÁGINA PRINCIPAL
══════════════════════════════════════════════════════════════════════════ */
// Real-template samples (spec §11), generated by `npm run samples`. Styles not yet
// standardized keep the abstract sketch.
const MUESTRAS: Partial<Record<EstiloId, { portada: string; completo: string }>> = {
  europass: { portada: '/muestras/europass-portada.png', completo: '/muestras/europass-completo.png' },
}
...
function Step2A({ selectedStyle, setSelectedStyle, isMobile, isPro, onGenerar, onUpgrade }: {
  ...
  // "Ver ejemplo": the full sample page of a style (spec §11).
  const [ejemplo, setEjemplo] = useState<EstiloId | null>(null)
  const ejemploMeta = ejemplo ? ESTILOS.find(e => e.id === ejemplo) ?? null : null
  useEffect(() => {
    if (!ejemplo) return
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') setEjemplo(null) }
  ...
  return (
    <div style={{ animation: 'fadeUp .25s var(--ease) both' }}>
      ...
      {isMobile
        ...
        : <EstilosGrid      selected={selectedStyle} onSelect={setSelectedStyle} isPro={isPro} onVerEjemplo={setEjemplo} />
      }

      {ejemplo && ejemploMeta && MUESTRAS[ejemplo] && (
        <div role="dialog" aria-modal="true" aria-label={`Ejemplo de CV ${ejemploMeta.nombre}`}
          onClick={() => setEjemplo(null)}
          style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(15,23,42,.55)', backdropFilter: 'blur(3px)', overflowY: 'auto', padding: '32px 16px' }}>
          <div onClick={e => e.stopPropagation()} style={{ maxWidth: 640, margin: '0 auto', position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, color: '#fff' }}>
              <span style={{ fontSize: 15, fontWeight: 700 }}>Ejemplo · {ejemploMeta.nombre}</span>
              <button type="button" onClick={() => setEjemplo(null)} aria-label="Cerrar ejemplo" autoFocus
                style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: 'rgba(255,255,255,.15)', color: '#fff', cursor: 'pointer', fontSize: 18, lineHeight: 1 }}>×</button>
            </div>
            {/* eslint-disable-next-line @next/next/no-img-element -- static sample in /public */}
            <img src={MUESTRAS[ejemplo]!.completo} alt={`Ejemplo completo de CV ${ejemploMeta.nombre}`}
              style={{ width: '100%', display: 'block', borderRadius: 6, boxShadow: '0 20px 60px rgba(15,23,42,.35)' }} />
            <p style={{ margin: '10px 0 0', fontSize: 12, color: 'rgba(255,255,255,.8)', textAlign: 'center' }}>
              Datos de ejemplo. Tu CV usará los datos de tu perfil.
            </p>
          </div>
        </div>
      )}
...
function EstilosGrid({ selected, onSelect, isPro, onVerEjemplo }: { selected: EstiloId | null; onSelect: (id: EstiloId) => void; isPro: boolean; onVerEjemplo: (id: EstiloId) => void }) {
  return (
    <div role="radiogroup" aria-label="Estilo de CV" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
      {ESTILOS.map(e => <EstiloCard key={e.id} estilo={e} selected={selected === e.id} onSelect={() => onSelect(e.id)} locked={e.isPro && !isPro} onVerEjemplo={() => onVerEjemplo(e.id)} />)}
    </div>
  )
}

function EstiloCard({ estilo, selected, onSelect, locked, onVerEjemplo }: {
  estilo: typeof ESTILOS[number]; selected: boolean; onSelect: () => void; locked: boolean; onVerEjemplo: () => void
}) {
  ...
  return (
    ...
    >
      {/* Área del preview CV: muestra real (mismo alto que el boceto) o boceto abstracto */}
      {muestra ? (
        <div style={{ height: 197, background: '#F8FAFC', flexShrink: 0, overflow: 'hidden' }}>
          {/* eslint-disable-next-line @next/next/no-img-element -- static sample in /public */}
          <img src={muestra.portada} alt={`Ejemplo de CV ${estilo.nombre}`} loading="lazy"
      ...
      <div style={{ padding: '14px 16px 16px', flex: 1 }}>
        ...
        <div style={{ fontSize: 11.5, color: '#64748B', lineHeight: 1.45, marginBottom: muestra ? 6 : 10 }}>{estilo.tagline}</div>
        {muestra && (
          <button type="button"
            onClick={e => { e.stopPropagation(); onVerEjemplo(); }}
            onKeyDown={e => e.stopPropagation()}
            style={{ background: 'none', border: 'none', padding: 0, marginBottom: 10, cursor: 'pointer', color: '#3B5BDB', fontSize: 12, fontWeight: 600 }}>
            Ver ejemplo →
          </button>
        )}
...
function CVThumbnail({ estilo }: { estilo: typeof ESTILOS[number] }) {
  ...
  if (muestra) {
    // eslint-disable-next-line @next/next/no-img-element -- static sample in /public
    return <img src={muestra.portada} alt="" loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'top', display: 'block' }} />
  }
  return (
