...
export async function POST(request: NextRequest) {
  try {
    ...
    const { cv_id, red, codigo } = body as { cv_id?: string; red?: CryptoNetwork; codigo?: string }

    if (!cv_id || !red) {
      return NextResponse.json({ error: 'Faltan parámetros: cv_id y red son requeridos.' }, { status: 400 })
    }
    ...
    const { precio_cv_unico: PRECIO_ORIGINAL } = await getConfig()

    // Discount code is always re-resolved server-side — never trust a client-sent price.
    let CV_UNICO_PRICE = PRECIO_ORIGINAL
    ...
    if (codigo?.trim()) {
      ...
      CV_UNICO_PRICE = calcularPrecioConDescuento(PRECIO_ORIGINAL, resuelto.porcentaje)
    ...
    // Check if already has a confirmed payment for this CV
    ...
    const nowPayment = await createNOWPayment({
      orderId:          `cv_${cv_id}_${Date.now()}`,
      orderDescription: 'CV Único — Momentum',
      priceAmount:      CV_UNICO_PRICE,
      payCurrency:      NETWORK_CURRENCY[red],
      ipnCallbackUrl:   `${siteUrl}/api/payments/webhook`,
    })

    // Persist to DB and update profile blocking field
    const [{ data: pago, error: pagoError }] = await Promise.all([
      admin.from('pagos').insert({
        user_id:                    user.id,
        cv_id,
        tipo:                       'cv_unico',
        monto:                      CV_UNICO_PRICE,
      ...
      // Mark CV as pending payment on profile so creation of new CVs is blocked
      admin.from('profiles')
        .update({ cv_pendiente_pago_id: cv_id })
        .eq('id', user.id),
    ])
